<!-- src/components/friend/FriendRequest.vue — 好友申请列表弹窗（深夜电台） -->
<template>
  <n-modal v-model:show="visible" preset="card" title="好友申请"
           class="max-w-md" :style="{ width: '90%', maxWidth: '28rem' }" @close="$emit('close')">
    <!-- 分类切换 -->
    <div class="flex gap-4 mb-3">
      <button :class="['pb-2 text-sm transition-colors', type === 'received' ? 'is-active' : '']"
              :style="type === 'received' ? 'color: var(--color-amber)' : 'color: var(--color-paper-faint)'"
              @click="switchType('received')">收到的申请</button>
      <button :class="['pb-2 text-sm transition-colors', type === 'sent' ? 'is-active' : '']"
              :style="type === 'sent' ? 'color: var(--color-amber)' : 'color: var(--color-paper-faint)'"
              @click="switchType('sent')">发出的申请</button>
    </div>

    <!-- 列表 -->
    <div class="max-h-[60vh] overflow-y-auto scroll-thin">
      <div v-if="loading" class="py-8 text-center text-sm" style="color: var(--color-paper-faint)">加载中...</div>
      <div v-else-if="items.length === 0" class="py-8 text-center text-sm" style="color: var(--color-paper-faint)">
        暂无申请
      </div>
      ...
      <div v-for="item in items" :key="item.id"
        ...
        <div class="flex-1 min-w-0">
          <div class="text-sm truncate" style="color: var(--color-paper)">
            {{ (type === 'received' ? item.sender : item.receiver)?.name }}
          </div>
          <div class="text-xs truncate" style="color: var(--color-paper-faint)">{{ item.message || '无附言' }}</div>
        </div>

        <template v-if="item.status === 'accepted'">
          <span class="text-xs" style="color: var(--color-paper-faint)">已同意</span>
          <n-button text size="tiny" @click="deleteRequest(item.id)" title="删除记录">
            <AppIcon name="x" :size="12" />
          </n-button>
        </template>
        <template v-else-if="item.status === 'rejected'">
          <span class="text-xs" style="color: var(--color-paper-faint)">已拒绝</span>
          <n-button text size="tiny" @click="deleteRequest(item.id)" title="删除记录">
            <AppIcon name="x" :size="12" />
          </n-button>
        </template>
        <template v-else-if="item.status === 'expired'">
          <span class="text-xs" style="color: var(--color-paper-faint)">已过期</span>
          <n-button text size="tiny" @click="deleteRequest(item.id)" title="删除记录">
            <AppIcon name="x" :size="12" />
          </n-button>
        </template>
