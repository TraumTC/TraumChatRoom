  ...
  <div class="h-screen flex overflow-hidden bg-white">
    ...
    <aside class="w-64 shrink-0 bg-gray-50 border-r border-gray-200 flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto scroll-thin min-h-0">
        ...
        <div v-for="user in onlineUsers" :key="user.name"
          ...
          <span v-if="getUnread(user.name) > 0"
                class="ml-auto min-w-4 h-4 px-1 rounded-full bg-red-500 text-white text-[10px] leading-none flex items-center justify-center">
            {{ getUnread(user.name) }}
          </span>
        </div>
      ...
      <div class="p-3 border-t border-gray-200 flex items-center gap-2">
        ...
        <div class="min-w-0">
          <div class="text-sm text-gray-800 truncate">{{ authStore.displayName }}</div>
    ...
    <main class="flex-1 flex flex-col min-w-0">
      ...
      <!-- 聊天区 -->
      <div ref="messageArea" class="flex-1 overflow-y-auto scroll-thin py-2"
           @scroll="handleScroll">
        <!-- 加载更多 -->
      ...
      <MessageInput :chat-type="isPrivateMode ? 'private' : 'group'"
                    :receiver="isPrivateMode ? currentChat.name : null"
...
async function startPrivateChat(user) {
  ...
  if (!chatStore.privateMessages[name] || chatStore.privateMessages[name].length === 0) {
    ...
    try {
      const res = await messageApi.getPrivateHistory(user.name, { size: pageSize })
